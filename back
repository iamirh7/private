...
# Function to load MIRROR_URL from config.env or script invocation
load_mirror_config() {
    # Default mirror URL
    MIRROR_URL="http://87.236.208.77"
    echo -e "${YELLOW}Loading mirror configuration...${PLAIN}"
    
    # Try to fetch config.env from the mirror server
    if curl -sSL --connect-timeout 10 --retry 3 -o /tmp/config.env "${MIRROR_URL}/config.env"; then
        echo -e "${GREEN}Successfully downloaded config.env${PLAIN}"
        if [ -s /tmp/config.env ]; then
            source /tmp/config.env
            echo -e "${GREEN}MIRROR_URL set to: $MIRROR_URL${PLAIN}"
        else
            echo -e "${RED}config.env is empty, using default MIRROR_URL: $MIRROR_URL${PLAIN}"
        fi
        rm -f /tmp/config.env
    else
        echo -e "${RED}Failed to download config.env, using default MIRROR_URL: $MIRROR_URL${PLAIN}"
    fi

    # If MIRROR_URL is set to BASE_URL, extract the IP from the script invocation
    if [ "$MIRROR_URL" = "BASE_URL" ]; then
        echo -e "${YELLOW}MIRROR_URL set to BASE_URL, attempting to extract from invocation...${PLAIN}"
        if [ -n "$BASH_SOURCE" ]; then
            SCRIPT_URL=$(ps -p $$ -o args= | grep -oP 'http://[^ ]+')
            if [ -n "$SCRIPT_URL" ]; then
                MIRROR_URL=$(echo "$SCRIPT_URL" | grep -oP 'http://[^/]+')
                echo -e "${GREEN}Extracted MIRROR_URL from invocation: $MIRROR_URL${PLAIN}"
            else
                echo -e "${RED}Could not extract URL from invocation, using default MIRROR_URL: $MIRROR_URL${PLAIN}"
            fi
        else
            echo -e "${RED}No BASH_SOURCE available, using default MIRROR_URL: $MIRROR_URL${PLAIN}"
        fi
    fi
}
...
# Function to detect Ubuntu version (for Ubuntu-based systems)
    ...
    if [[ -f /etc/os-release ]]; then
        ...
        if [[ "$ID" == "ubuntu" ]]; then
            case $VERSION_ID in
                "24.04") echo "u24" ;;
                "22.04") echo "u22" ;;
                *) echo -e "${RED}Unsupported Ubuntu version: $VERSION_ID${PLAIN}"; sleep 2; exit 1 ;;
            esac
        else
            echo "non_ubuntu"
        fi
    else
        echo -e "${RED}Failed to check the system OS!${PLAIN}" >&2
        exit 1
    fi
}

# Function to download and install .deb package from mirror (for Ubuntu-based systems)
install_deb_from_mirror() {
    local pkg_name="$1"
    local arch="$2"
    local ubuntu_version="$3"
    local deb_file="${pkg_name}_${arch}.deb"
    local deb_url="${MIRROR_URL}/pkg/${ubuntu_version}/${deb_file}"
    local temp_dir=$(mktemp -d)

    echo -e "${YELLOW}Attempting to download ${pkg_name} from mirror: ${deb_url}${PLAIN}"
    if curl -sSL --connect-timeout 10 --retry 3 -o "${temp_dir}/${deb_file}" "${deb_url}"; then
        if [ -s "${temp_dir}/${deb_file}" ]; then
            echo -e "${GREEN}Installing ${pkg_name} from ${deb_file}...${PLAIN}"
            if dpkg -i "${temp_dir}/${deb_file}" >/dev/null 2>&1; then
                apt-get install -f -y >/dev/null 2>&1
                echo -e "${GREEN}${pkg_name} installed successfully${PLAIN}"
                rm -rf "${temp_dir}"
                return 0
            else
                echo -e "${RED}Failed to install ${pkg_name} from ${deb_file}${PLAIN}"
                rm -rf "${temp_dir}"
                return 1
            fi
        else
            echo -e "${RED}Downloaded ${deb_file} is empty or corrupted${PLAIN}"
            rm -rf "${temp_dir}"
            return 1
        fi
    else
        echo -e "${RED}Failed to download ${deb_file} from mirror${PLAIN}"
        rm -rf "${temp_dir}"
        return 1
    fi
}
...
install_with_package_manager() {
    local pkg_name="$1"
    echo -e "${YELLOW}Falling back to system package manager for ${pkg_name}...${PLAIN}"
    if command -v apt-get &> /dev/null; then
        apt-get update >/dev/null 2>&1
        apt-get install -y "${pkg_name}" >/dev/null 2>&1 && echo -e "${GREEN}${pkg_name} installed successfully${PLAIN}" && return 0
    else
        echo -e "${RED}Unsupported package manager. Please install ${pkg_name} manually.${PLAIN}"
        echo
        press_key
        exit 1
    fi
...
install_unzip() {
    if ! command -v unzip &> /dev/null; then
        ...
        if [[ "$ubuntu_version" != "non_ubuntu" ]]; then
            install_deb_from_mirror "unzip" "${system_arch}" "${ubuntu_version}" || install_with_package_manager "unzip"
        else
...
install_jq() {
    if ! command -v jq &> /dev/null; then
        ...
        local ubuntu_version=$(detect_ubuntu_version)
        if [[ "$ubuntu_version" != "non_ubuntu" ]]; then
            install_deb_from_mirror "jq" "${system_arch}" "${ubuntu_version}" || install_with_package_manager "jq"
        else
...
download_and_extract_backhaul() {
    ...
    case "$ARCH" in
        amd64)
            DOWNLOAD_URL="${MIRROR_URL}/backhaul/backhaul_amd64.tar.gz"
            FALLBACK_URL="http://87.236.208.77/backhaul/backhaul_amd64.tar.gz"
            ;;
        arm64)
            DOWNLOAD_URL="${MIRROR_URL}/backhaul/backhaul_arm64.tar.gz"
            FALLBACK_URL="http://87.236.208.77/backhaul/backhaul_arm64.tar.gz"
            ;;
        *)
    ...
    sleep 1
    if curl -sSL --connect-timeout 10 --retry 3 -o "$DOWNLOAD_DIR/backhaul.tar.gz" "$DOWNLOAD_URL"; then
        if [ -s "$DOWNLOAD_DIR/backhaul.tar.gz" ]; then
            echo -e "${GREEN}Backhaul downloaded successfully from mirror.${NC}"
        else
            echo -e "${RED}Downloaded backhaul.tar.gz is empty or corrupted${NC}"
            rm -f "$DOWNLOAD_DIR/backhaul.tar.gz"
            echo -e "${YELLOW}Falling back to original source: $FALLBACK_URL${NC}"
            if curl -sSL --connect-timeout 10 --retry 3 -o "$DOWNLOAD_DIR/backhaul.tar.gz" "$FALLBACK_URL"; then
                if [ -s "$DOWNLOAD_DIR/backhaul.tar.gz" ]; then
                    echo -e "${GREEN}Backhaul downloaded successfully from original source.${NC}"
                else
                    echo -e "${RED}Downloaded backhaul.tar.gz from original source is empty or corrupted${NC}"
                    rm -f "$DOWNLOAD_DIR/backhaul.tar.gz"
                    echo -e "${RED}Downloading Backhaul failed, please check your server connectivity or file availability${NC}"
                    sleep 1
                    exit 1
                fi
            else
                echo -e "${RED}Failed to download backhaul.tar.gz from original source${NC}"
                sleep 1
                exit 1
            fi
        fi
